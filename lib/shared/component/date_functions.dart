...
int currentAuctionCounter({required int endTime}){
 ...
 Duration countDuration=DateTime.fromMillisecondsSinceEpoch(endTime*1000).difference(DateTime.now());
 DateTime timeOfDuration=DateTime.now().add(countDuration);
 int myCounter=timeOfDuration.millisecondsSinceEpoch;
  return myCounter;
 }

 final DatabaseReference reference = FirebaseDatabase.instance.ref().child('chat_room');
 String? newKey;

 String nextAuctionLeftTime({ required int adTime, required BuildContext context}) {
   int dateMints = ((DateTime.fromMillisecondsSinceEpoch(adTime*1000).minute)-DateTime.now().minute).abs() ;
   int dateHours = ((DateTime.fromMillisecondsSinceEpoch(adTime*1000).hour)-DateTime.now().hour).abs();
   int dateDays =((DateTime.fromMillisecondsSinceEpoch(adTime*1000).day)-DateTime.now().day).abs();
   int dateMonth = ((DateTime.fromMillisecondsSinceEpoch(adTime*1000).month)-DateTime.now().month).abs();
   int dateYear = ((DateTime.fromMillisecondsSinceEpoch(adTime*1000).year)-DateTime.now().year).abs();
 ...
 String notificationDate(int timeInEpoch){
   String date='${DateTime.fromMillisecondsSinceEpoch(timeInEpoch*1000).day}/${DateTime.fromMillisecondsSinceEpoch(timeInEpoch*1000).month}';
   return date;
 }
 String notificationTime(int timeInEpoch){

   String time1=DateFormat.jm().format(DateTime.fromMillisecondsSinceEpoch(timeInEpoch*1000));
   return time1;
 }
