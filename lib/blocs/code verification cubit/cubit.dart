...
class VerificationCodeCubit extends Cubit<VerificationCodeStates> {
  ...
  CountdownTimerController initialTimer(){
    timerController=CountdownTimerController(endTime: DateTime.now().millisecondsSinceEpoch+(31*1000),onEnd: (){timeFinished();});
  ...
  void postFcmToken(){
    ...
    DioHelper.postData(
     ...
     if(value.data['model-state']==1)
      emit(PostFcmTokenSuccessfulState());
    }).catchError((onError){
      emit(PostFcmTokenErrorState());
    });
  }

  void insureCodeVerification({required String verificationId,required String smsCode,required String phoneNumber}){
