...
class InComingAuctionCubit extends Cubit<InComingAuctionStates>{
 ...
 Future<void> getIncomingAuction({bool isRefresh=false})async{
    ...
    DioHelper.getData(
      url: 'postponedAuctions?page=$currentPage',
      headers: {
      ...
      if(value.statusCode==200){
        ...
        if(inComingAuctionModel!.modelState==1){
          print('api list${inComingAuctionModel!.data!.nextAuctions!.data.length}');
          if(isRefresh){
            nextFollowedList={};
            allData=inComingAuctionModel!.data!.nextAuctions!.data;
            for(var element in inComingAuctionModel!.data!.nextAuctions!.data){
