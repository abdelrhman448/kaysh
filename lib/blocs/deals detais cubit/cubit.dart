...
ScrollController scrollController=ScrollController();
  ...
  Future<void> increaseHighestPrice()async {
   ...
   void getAuctionDetails({required int auctionId}){

         DioHelper.getData(
           url:'auctionDetails?auction_id=$auctionId',
           headers: {
...
// send bid to firebase
  void sendBid({required String value,required int userId, required int auctionId}){
    emit(SendAuctionLoadingState());
    AllBidsModel allBidsModel=AllBidsModel(bidValue: value,time: DateTime.now().toString(), userId: userId.toString());
    FirebaseFirestore.instance.collection('auction').doc(auctionId.toString()).collection('bids').add(allBidsModel.toMap()).then((value) {
  ...
  void getAllAuctionBids(int id){
    FirebaseFirestore.instance.collection('auction').doc(id.toString()).collection('bids').orderBy('time',descending: true)
